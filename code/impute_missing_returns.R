impute_missing_returns <- function(return_mat, impute_returns_method = "NONE"){
    ...
    if( impute_returns_method  == "Average") {
        ...
        return_mat <-
            ...
            ungroup() %>%
            mutate(Returns = coalesce(Returns, Avg)) %>% dplyr:::select(-Avg) %>% spread(Stocks, Returns)
        ...
        if( impute_returns_method  == "Drawn_Distribution_Own") {
            ...
            return_mat <-
                ...
                dplyr:::select(-Random_Draws) %>% ungroup() %>% spread(Stocks, Returns)
            return(return_mat)
        } else
            ...
            if( impute_returns_method  == "Drawn_Distribution_Collective") {
                ...
                return_mat <-
                    ...
                    mutate(Returns = coalesce(Returns, Random_Draws)) %>% dplyr:::select(-Random_Draws) %>% spread(Stocks, Returns)
                return(return_mat)
            } else
